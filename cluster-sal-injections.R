...
p <- ggplot(centers_df, aes(time, bpi_intensity)) +
    ...
    geom_line(data=aug_data,
              mapping=aes(x=time,
                          ...
                          group=id),
              alpha=0.3) +
    labs(x="time",y="bpi_intensity",title=sprintf("clustering of saline data"));
ggsave(filename="figures/clustering_saline.png", plot=p);
...
p <- ggplot(centers_df, aes(time, bpi_intensity)) +
    ...
    geom_line(data=aug_data,
              mapping=aes(x=time,
                          ...
                          group=id),
              alpha=0.3) +
    labs(x="time",y="bpi_intensity",title=sprintf("clustering of saline data (normed)"));
ggsave(filename="figures/clustering_saline_norm.png", plot=p);
