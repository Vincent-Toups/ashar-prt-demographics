...
p <- ggplot(centers_df, aes(time, bpi_intensity)) +
    ...
    geom_line(data=aug_data,
              mapping=aes(x=time,
                          ...
                          group=id),
              alpha=0.3) +
    labs(x="time",y="bpi_intensity",title=sprintf("soc clusters"));
ggsave(filename="figures/clustering_soc.png", plot=p);
...
p <- ggplot(centers_df, aes(time, bpi_intensity)) +
    ...
    geom_line(data=aug_data,
              mapping=aes(x=time,
                          ...
                          group=id),
              alpha=0.3) +
    labs(x="time",y="bpi_intensity",title=sprintf("soc clusters (normed)"));
ggsave(filename="figures/clustering_soc_norm.png", plot=p);
